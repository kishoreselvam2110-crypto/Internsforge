...
def list_jobs(db: Client = Depends(get_db_client), user_id: str = Depends(get_user_id_from_token)):
    """
    List all job descriptions for the authenticated user.
    """
    try:
        response = db.table("job_descriptions").select("id, title, description, skills, experience_years, education_level, created_at, user_id").eq("user_id", user_id).order("created_at", desc=True).execute()
        return response.data
    except Exception as e:
        raise HTTPException(status_code=400, detail=f"Database error: {str(e)}")

@router.get("/{job_id}", response_model=JobDescriptionResponse)
def get_job(job_id: str, db: Client = Depends(get_db_client), user_id: str = Depends(get_user_id_from_token)):
    """
    Get a specific job description by ID.
    """
    try:
        response = db.table("job_descriptions").select("id, title, description, skills, experience_years, education_level, created_at, user_id").eq("id", job_id).eq("user_id", user_id).execute()
        if not response.data:
